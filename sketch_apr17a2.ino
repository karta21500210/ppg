...
void loop() {
  ...
  while (particleSensor.available()) {
    ...
    if (time_t1 - top_temp <= heart_t_treth * 0.7 and time_t1 - top_temp > heart_t_treth * 0.2)
    {
      if (irbuffer_d[flag - 1 >= 0 ? flag - 1 : buffer_length - 1] >= 0 and irbuffer_d[flag] < 0 )
      {
        Serial.print(140);
      }//因為記憶體空間不夠只能用這樣
      else if (irbuffer_d[flag] < 0 and irbuffer_d[flag - 1 >= 0 ? flag - 1 : buffer_length - 1] < 0 \
      ...
      Serial.print(",");
      if (redbuffer_d[flag - 1 >= 0 ? flag - 1 : buffer_length - 1] >= 0 and redbuffer_d[flag] < 0)
      {
        Serial.println(-140);
      }
      else if (redbuffer_d[flag] < 0 and redbuffer_d[flag - 1 >= 0 ? flag - 1 : buffer_length - 1] < 0 \
